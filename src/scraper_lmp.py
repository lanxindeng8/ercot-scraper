#!/usr/bin/env python3
"""
LMP by Settlement Point Scraper

Fetches LMP (Locational Marginal Price) data from ERCOT and writes to InfluxDB.
"""
...
def main():
    """Main scraper function for LMP data"""
    print(f"Starting LMP scraper at {datetime.utcnow().isoformat()}")
    ...
    try:
        ...
        last_timestamp = influxdb.get_last_timestamp("lmp_by_settlement_point")
        ...
        print("Fetching LMP data from ERCOT...")
        total_records = 0
        ...
        for page_data in ercot.fetch_lmp_by_settlement_point(start_date):
            ...
            # Write to InfluxDB
            points_written = influxdb.write_lmp_data(page_data)
        ...
        print(f"Error in LMP scraper: {str(e)}")
        import traceback
        traceback.print_exc()
        return 1
