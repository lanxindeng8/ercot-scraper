#!/usr/bin/env python3
"""
SPP Day Ahead Hourly Scraper

Fetches Day-Ahead Settlement Point Prices from ERCOT and writes to InfluxDB.
"""
...
def main():
    """Main scraper function for SPP data"""
    print(f"Starting SPP scraper at {datetime.utcnow().isoformat()}")
    ...
    try:
        ...
        last_timestamp = influxdb.get_last_timestamp("spp_day_ahead_hourly")
        ...
        print("Fetching SPP Day Ahead data from ERCOT...")
        total_records = 0
        ...
        for page_data in ercot.fetch_spp_day_ahead_hourly(start_date):
            ...
            # Write to InfluxDB
            points_written = influxdb.write_spp_day_ahead_data(page_data)
        ...
        print(f"Error in SPP scraper: {str(e)}")
        import traceback
        traceback.print_exc()
        return 1
